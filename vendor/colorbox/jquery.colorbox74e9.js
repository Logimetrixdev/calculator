...
(function ($, document, window) {
		...
		if (!$box && document.body) {
			...
			$wrap.append( // The 3x3 Grid that makes up Colorbox
				...
				$tag(div, false, 'clear:left').append(
					...
					$bottomRightBorder = $tag(div, "BottomRight")
				)
	...
	publicMethod.position = function (speed, loadedCallback) {
		...
		function modalDimensions() {
			...
			if ($topBorder[0].style.width != '' &&
					($topLeftBorder.width() != $leftBorder.width() || $rightBorder.width() != $bottomRightBorder.width())) {

				var sizeDifference = ($topLeftBorder.width() - $leftBorder.width()) + ($bottomRightBorder.width() - $rightBorder.width());
				$topBorder[0].style.width = ($topBorder.width() - sizeDifference) + 'px';
				$bottomBorder[0].style.width = ($bottomBorder.width() - sizeDifference) + 'px';
			}			
...
function attachRateDetailsDialog(link, href, contact, forceMobilePopup) {
	...
	// force use of original lead form, otherwise logic gets unnecessarily complicated
	var linkHref = href ? href : link.attr("href");
	link.attr("href", appendQueryParams(linkHref, {originalForm: true}));
