...
var fdSlider = (function() {
        ...
        function fdRange(options) {
                ...
                // Get tabindex from element
                tabIndex = inp[!/*@cc_on!@*/false ? "tabIndex" : "tabindex"];
                ...
                function setInputValue(val) {
                        ...
                        if(tagName == "select") {
                                ...
                                if(val != "") {
                                        ...
                                        // Fix to prevent rounding past max value
                                        val = (val > max? max : val).toFixed(precision);
                                };
                                if(inp.value === val) {
                                        updateAriaValues();                                                          
                                        return;
                ...
                function setSliderRange(newMin, newMax) {
                        ...
                        // Hack: actually implement the function
                        min         = newMin;
                        max         = newMax;
                        range       = Math.abs(max - min);
                        steps       = Math.ceil(range / step);
        ...
        return {                                           
                ...
                addCallback: function(id, event, fn) {
                	// addEvent() does not work with callbacks (events that are called when the slider is not an HTML5 shim)
                	if(!sliderExists(id)) {
                		return false;
                	}
                	var ref = sliders[id].callbacks;
                	if(typeof(ref[event]) == 'undefined') {
                		ref[event] = [];
                	}
                	ref[event].push(fn);
                }
